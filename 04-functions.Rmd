...
These functions essentially repeat the previous steps for the example model. The parameters can be imputed into the functions to give us a resulting data frame and plot to visualise for each unique combination of handedness, count, and pitch type. 
...
gf1 <- gam(re ~ s(x, y), data = hand_pitch_count_df, method = "REML") #k=40
...
gg <- gg %>% mutate(lp2 = (lp - min(lp)) / (max(lp) - min(lp))) %>% select(-lp) %>%
  mutate(lp2 = 1-lp2)
...
location_quality_plot <- function(df) {
  ...
  df <- df %>% mutate(label = paste(hand, pitch_type, count, sep = " "))
    ggplot() +
      geom_tile(data= df, aes(x,y, fill = lp2), show.legend = FALSE) +
